...
__The major assumptions for logistic regression includes 
```{r,warning=FALSE,message=FALSE}
# Lack of Fit Plot
train_pi <- predict(logit_model, interval="predict")
intuitve_pi <- predict()

model_data <- cbind(train, train_pi)
model_plot <- ggplot(model_data, aes(fit,Life.expectancy)) + geom_point() + stat_smooth(method=lm) + 
  geom_line(aes(y=lwr), color="red", linetype="dashed")+
  geom_line(aes(y=upr), color="red", linetype="dashed")+
  labs(title="Predicted vs Actual Life Expectancy",
       caption="Source: Life Expectancy (WHO)",
       x="Predicted Value")
model_plot
...
```{r,warning=FALSE,message=FALSE}
# Viewing Model Selection Prediction Specificity/Sensivity
library(ROCR)
...
# Adding complexity to the model: what if we were to take our highly correlated values and instead apply an interaction?
